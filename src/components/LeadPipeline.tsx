        ...
        <table>
          ...
          <tbody>
            {paged.map(lead => {
              ...
              return (
                <tr key={lead.id}>
                  <td data-label="Lead"><div className="lead"><b>{(lead.company || '?')[0].toUpperCase()}</b><span><strong>{lead.company || 'Untitled lead'}</strong><small>{lead.contact_name || 'No contact name'}{lead.niche ? ' · ' + lead.niche : ''}</small></span></div></td>
